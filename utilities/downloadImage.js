...
module.exports = async (url, userId) => {
	...
	if (head.headers["content-length"] > process.env.MAX_FILE_SIZE_UPLOAD * 1024 * 1024) {
		return new AppError(
	...
	if (!contentType) {
		return new AppError("This url does not provide mimetype. Download is not possible.", 400);
	}
